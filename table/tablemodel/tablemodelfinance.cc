...
TableModelFinance::TableModelFinance(const Info* info, const SectionRule* section_rule, QSharedPointer<Sql> sql, int node_id, bool node_rule, QObject* parent)
...
TableModelFinance::~TableModelFinance() { RecycleTrans(trans_list_); }

void TableModelFinance::RNodeRule(int node_id, bool node_rule)
{
...
void TableModelFinance::RAppendOne(CSPCTrans& trans)
{
...
void TableModelFinance::RRetrieveOne(CSPTrans& trans)
{
    auto row { trans_list_.size() };
...
bool TableModelFinance::AppendOne(const QModelIndex& parent)
{
...
void TableModelFinance::RUpdateBalance(int node_id, int trans_id)
{
    if (node_id_ != node_id)
        return;
...
void TableModelFinance::RRemoveOne(int node_id, int trans_id)
{
    if (node_id_ != node_id)
        return;
...
bool TableModelFinance::DeleteOne(int row, const QModelIndex& parent)
{
    if (row <= -1)
        return false;
...
void TableModelFinance::RMoveMulti(int old_node_id, int new_node_id, const QList<int>& trans_id_list)
{
...
void TableModelFinance::RRemoveMulti(const QMultiHash<int, int>& node_trans)
{
...
bool TableModelFinance::RemoveMulti(const QList<int>& trans_id_list)
{
    int min_row {};
    int trans_id {};
...
QModelIndex TableModelFinance::TransIndex(int trans_id) const
{
    int row { 0 };
...
void TableModelFinance::UpdateState(Check state)
{
    for (auto& trans : trans_list_) {
        switch (state) {
        case Check::kAll:
...
int TableModelFinance::NodeRow(int node_id) const
{
    int row { 0 };
...
void TableModelFinance::RecycleTrans(SPTransList& list)
{
    for (auto& trans : list)
        TransPool::Instance().Recycle(trans);
...
QModelIndex TableModelFinance::index(int row, int column, const QModelIndex& parent) const
{
...
QModelIndex TableModelFinance::parent(const QModelIndex& index) const
{
    Q_UNUSED(index);
    return QModelIndex();
}

int TableModelFinance::rowCount(const QModelIndex& parent) const
{
    Q_UNUSED(parent);
    return trans_list_.size();
}

int TableModelFinance::columnCount(const QModelIndex& parent) const
{
    Q_UNUSED(parent);
...
bool TableModelFinance::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    case PartTableColumn::kDescription:
        ...
        break;
    case PartTableColumn::kRatio:
        rat_changed = UpdateFXRate(trans, value.toDouble());
        break;
    case PartTableColumn::kRelatedNode:
...
bool TableModelFinance::UpdateDateTime(SPTrans& trans, CString& value)
{
...
bool TableModelFinance::UpdateDescription(SPTrans& trans, CString& value)
{
...
bool TableModelFinance::UpdateCode(SPTrans& trans, CString& value)
{
    if (*trans->code == value)
        return false;
...
bool TableModelFinance::UpdateState(SPTrans& trans, bool value)
{
...
bool TableModelFinance::UpdateDebit(SPTrans& trans, double value)
{
...
bool TableModelFinance::UpdateCredit(SPTrans& trans, double value)
{
    double credit { *trans->credit };
    const double tolerance { std::pow(10, -section_rule_->value_decimal - 2) };
...
bool TableModelFinance::UpdateFXRate(SPTrans& trans, double value)
{
...
bool TableModelFinance::UpdateRelatedNode(SPTrans& trans, int value)
{
...
double TableModelFinance::Balance(bool node_rule, double debit, double credit) { return node_rule ? credit - debit : debit - credit; }

void TableModelFinance::AccumulateBalance(const SPTransList& list, int row, bool node_rule)
{
...
QVariant TableModelFinance::headerData(int section, Qt::Orientation orientation, int role) const
{
...
bool TableModelFinance::AppendMulti(int node_id, const QList<int>& trans_id_list)
{
    auto row { trans_list_.size() };
