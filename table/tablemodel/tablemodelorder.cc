...
TableModelOrder::TableModelOrder(const Info* info, const SectionRule* section_rule, QSharedPointer<Sql> sql, int node_id, bool node_rule, QObject* parent)
...
TableModelOrder::~TableModelOrder() { RecycleTrans(trans_list_); }

void TableModelOrder::RNodeRule(int node_id, bool node_rule)
{
...
void TableModelOrder::RAppendOne(CSPCTrans& trans)
{
...
void TableModelOrder::RRetrieveOne(CSPTrans& trans)
{
    auto row { trans_list_.size() };
...
bool TableModelOrder::AppendOne(const QModelIndex& parent)
{
...
void TableModelOrder::RUpdateBalance(int node_id, int trans_id)
{
    if (node_id_ != node_id)
        return;
...
void TableModelOrder::RRemoveOne(int node_id, int trans_id)
{
    if (node_id_ != node_id)
        return;
...
bool TableModelOrder::DeleteOne(int row, const QModelIndex& parent)
{
    if (row <= -1)
        return false;
...
void TableModelOrder::RMoveMulti(int old_node_id, int new_node_id, const QList<int>& trans_id_list)
{
...
void TableModelOrder::RRemoveMulti(const QMultiHash<int, int>& node_trans)
{
...
bool TableModelOrder::RemoveMulti(const QList<int>& trans_id_list)
{
    int min_row {};
    int trans_id {};
...
QModelIndex TableModelOrder::TransIndex(int trans_id) const
{
    int row { 0 };
...
void TableModelOrder::UpdateState(Check state)
{
    for (auto& trans : trans_list_) {
        switch (state) {
        case Check::kAll:
...
int TableModelOrder::NodeRow(int node_id) const
{
    int row { 0 };
...
void TableModelOrder::RecycleTrans(SPTransList& list)
{
    for (auto& trans : list)
        TransPool::Instance().Recycle(trans);
...
QModelIndex TableModelOrder::parent(const QModelIndex& index) const
{
    Q_UNUSED(index);
    return QModelIndex();
}
...
bool TableModelOrder::UpdateDateTime(SPTrans& trans, CString& value)
{
...
bool TableModelOrder::UpdateDescription(SPTrans& trans, CString& value)
{
...
bool TableModelOrder::UpdateCode(SPTrans& trans, CString& value)
{
    if (*trans->code == value)
        return false;
...
bool TableModelOrder::UpdateState(SPTrans& trans, bool value)
{
...
bool TableModelOrder::UpdateDebit(SPTrans& trans, double value)
{
...
bool TableModelOrder::UpdateCredit(SPTrans& trans, double value)
{
    double credit { *trans->credit };
    const double tolerance { std::pow(10, -section_rule_->value_decimal - 2) };
...
bool TableModelOrder::UpdateRatio(SPTrans& trans, double value)
{
...
bool TableModelOrder::UpdateRelatedNode(SPTrans& trans, int value)
{
...
double TableModelOrder::Balance(bool node_rule, double debit, double credit) { return node_rule ? credit - debit : debit - credit; }

void TableModelOrder::AccumulateBalance(const SPTransList& list, int row, bool node_rule)
{
...
bool TableModelOrder::AppendMulti(int node_id, const QList<int>& trans_id_list)
{
    auto row { trans_list_.size() };
