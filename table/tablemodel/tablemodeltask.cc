...
TableModelTask::TableModelTask(const Info* info, const SectionRule* section_rule, QSharedPointer<Sql> sql, int node_id, bool node_rule, QObject* parent)
...
TableModelTask::~TableModelTask() { RecycleTrans(trans_list_); }

void TableModelTask::RNodeRule(int node_id, bool node_rule)
{
...
void TableModelTask::RAppendOne(CSPCTrans& trans)
{
...
void TableModelTask::RRetrieveOne(CSPTrans& trans)
{
    auto row { trans_list_.size() };
...
bool TableModelTask::AppendOne(const QModelIndex& parent)
{
...
void TableModelTask::RUpdateBalance(int node_id, int trans_id)
{
    if (node_id_ != node_id)
        return;
...
void TableModelTask::RRemoveOne(int node_id, int trans_id)
{
    if (node_id_ != node_id)
        return;
...
bool TableModelTask::DeleteOne(int row, const QModelIndex& parent)
{
    if (row <= -1)
        return false;
...
void TableModelTask::RMoveMulti(int old_node_id, int new_node_id, const QList<int>& trans_id_list)
{
...
void TableModelTask::RRemoveMulti(const QMultiHash<int, int>& node_trans)
{
...
bool TableModelTask::RemoveMulti(const QList<int>& trans_id_list)
{
    int min_row {};
    int trans_id {};
...
QModelIndex TableModelTask::TransIndex(int trans_id) const
{
    int row { 0 };
...
void TableModelTask::UpdateState(Check state)
{
    for (auto& trans : trans_list_) {
        switch (state) {
        case Check::kAll:
...
int TableModelTask::NodeRow(int node_id) const
{
    int row { 0 };
...
void TableModelTask::RecycleTrans(SPTransList& list)
{
    for (auto& trans : list)
        TransPool::Instance().Recycle(trans);
...
QModelIndex TableModelTask::index(int row, int column, const QModelIndex& parent) const
{
...
QModelIndex TableModelTask::parent(const QModelIndex& index) const
{
    Q_UNUSED(index);
    return QModelIndex();
}
...
bool TableModelTask::UpdateDateTime(SPTrans& trans, CString& value)
{
...
bool TableModelTask::UpdateDescription(SPTrans& trans, CString& value)
{
...
bool TableModelTask::UpdateCode(SPTrans& trans, CString& value)
{
    if (*trans->code == value)
        return false;
...
bool TableModelTask::UpdateState(SPTrans& trans, bool value)
{
...
bool TableModelTask::UpdateDebit(SPTrans& trans, double value)
{
...
bool TableModelTask::UpdateCredit(SPTrans& trans, double value)
{
    double credit { *trans->credit };
    const double tolerance { std::pow(10, -section_rule_->value_decimal - 2) };
...
bool TableModelTask::UpdateUnitCost(SPTrans& trans, double value)
{
...
bool TableModelTask::UpdateRelatedNode(SPTrans& trans, int value)
{
...
double TableModelTask::Balance(bool node_rule, double debit, double credit) { return node_rule ? credit - debit : debit - credit; }

void TableModelTask::AccumulateBalance(const SPTransList& list, int row, bool node_rule)
{
...
bool TableModelTask::AppendMulti(int node_id, const QList<int>& trans_id_list)
{
    auto row { trans_list_.size() };
