...
TreeModelFinance::~TreeModelFinance() { RecycleNode(node_hash_); }
...
void TreeModelFinance::UpdateNode(const Node* tmp_node)
{
    if (!tmp_node)
        return;
...
void TreeModelFinance::UpdateSeparator(CString& separator)
{
...
bool TreeModelFinance::RRemoveNode(int node_id)
{
...
void TreeModelFinance::IniTree(NodeHash& node_hash, StringHash& leaf_path, StringHash& branch_path)
{
    sql_->Tree(node_hash);
...
QString TreeModelFinance::CreatePath(const Node* node) const
{
    if (!node || node == root_)
        return QString();
...
void TreeModelFinance::UpdateBranchUnit(Node* node) const
{
...
void TreeModelFinance::RecycleNode(NodeHash& node_hash)
{
    for (auto& node : std::as_const(node_hash))
...
QModelIndex TreeModelFinance::index(int row, int column, const QModelIndex& parent) const
{
...
int TreeModelFinance::rowCount(const QModelIndex& parent) const { return GetNode(parent)->children.size(); }
...
bool TreeModelFinance::UpdateDescription(Node* node, CString& value)
{
...
bool TreeModelFinance::UpdateNote(Node* node, CString& value)
{
    if (node->note == value)
        return false;
...
void TreeModelFinance::UpdatePath(const Node* node)
{
    QQueue<const Node*> queue {};
    queue.enqueue(node);
...
bool TreeModelFinance::UpdateRule(Node* node, bool value)
{
...
bool TreeModelFinance::UpdateCode(Node* node, CString& value)
{
    if (node->code == value)
        return false;
...
void TreeModelFinance::SortIterative(Node* node, std::function<bool(const Node*, const Node*)> Compare)
{
    if (!node)
        return;
...
Node* TreeModelFinance::GetNode(const QModelIndex& index) const
{
...
QString TreeModelFinance::Path(int node_id) const
{
...
bool TreeModelFinance::IsDescendant(Node* lhs, Node* rhs) const
{
    if (!lhs || !rhs || lhs == rhs)
        return false;
...
bool TreeModelFinance::InsertRow(int row, const QModelIndex& parent, Node* node)
{
    if (row <= -1)
        return false;
...
bool TreeModelFinance::RemoveRow(int row, const QModelIndex& parent)
{
...
bool TreeModelFinance::UpdateLeafTotal(const Node* node)
{
    if (!node || node->branch)
        return false;
...
QVariant TreeModelFinance::headerData(int section, Qt::Orientation orientation, int role) const
{
...
QMimeData* TreeModelFinance::mimeData(const QModelIndexList& indexes) const
{
...
bool TreeModelFinance::canDropMimeData(const QMimeData* data, Qt::DropAction action, int row, int column, const QModelIndex&) const
{
    Q_UNUSED(row);
    Q_UNUSED(column);
