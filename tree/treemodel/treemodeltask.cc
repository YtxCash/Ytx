...
TreeModelTask::~TreeModelTask() { RecycleNode(node_hash_); }
...
void TreeModelTask::UpdateSeparator(CString& separator)
{
...
bool TreeModelTask::RRemoveNode(int node_id)
{
...
void TreeModelTask::IniTree(NodeHash& node_hash, StringHash& leaf_path, StringHash& branch_path)
{
    sql_->Tree(node_hash);
...
QString TreeModelTask::CreatePath(const Node* node) const
{
    if (!node || node == root_)
        return QString();
...
void TreeModelTask::UpdateBranchUnit(Node* node) const
{
...
QModelIndex TreeModelTask::index(int row, int column, const QModelIndex& parent) const
{
...
int TreeModelTask::rowCount(const QModelIndex& parent) const { return GetNode(parent)->children.size(); }

bool TreeModelTask::UpdateBranch(Node* node, bool value)
{
    int node_id { node->id };
...
bool TreeModelTask::UpdateDescription(Node* node, CString& value)
{
...
bool TreeModelTask::UpdateNote(Node* node, CString& value)
{
    if (node->note == value)
        return false;
...
void TreeModelTask::UpdatePath(const Node* node)
{
    QQueue<const Node*> queue {};
    queue.enqueue(node);
...
bool TreeModelTask::UpdateRule(Node* node, bool value)
{
...
bool TreeModelTask::UpdateCode(Node* node, CString& value)
{
    if (node->code == value)
        return false;
...
void TreeModelTask::SortIterative(Node* node, std::function<bool(const Node*, const Node*)> Compare)
{
    if (!node)
        return;
...
Node* TreeModelTask::GetNode(const QModelIndex& index) const
{
...
QString TreeModelTask::Path(int node_id) const
{
...
bool TreeModelTask::IsDescendant(Node* lhs, Node* rhs) const
{
    if (!lhs || !rhs || lhs == rhs)
        return false;
...
bool TreeModelTask::InsertRow(int row, const QModelIndex& parent, Node* node)
{
    if (row <= -1)
        return false;
...
bool TreeModelTask::RemoveRow(int row, const QModelIndex& parent)
{
...
bool TreeModelTask::UpdateLeafTotal(const Node* node)
{
    if (!node || node->branch)
        return false;
...
QVariant TreeModelTask::headerData(int section, Qt::Orientation orientation, int role) const
{
...
QMimeData* TreeModelTask::mimeData(const QModelIndexList& indexes) const
{
...
bool TreeModelTask::canDropMimeData(const QMimeData* data, Qt::DropAction action, int row, int column, const QModelIndex&) const
{
    Q_UNUSED(row);
    Q_UNUSED(column);
